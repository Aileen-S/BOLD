...
if (opt$country == 'Canada') {
  country <- canada_total
}
...
Count <- function(df, fam) {
  fam <- as.character(fam)
  subset_data <- subset(df, family == fam)
